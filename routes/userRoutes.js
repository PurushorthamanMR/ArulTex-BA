 ...
 * Login user
 * POST /user/login
 * Body (JSON): { "username" or "emailAddress" or "email": "...", "password": "..." }
...
router.post('/login', async (req, res) => {
  try {
    ...
    const body = req.body || {};
    const loginDto = {
      username: body.username || body.emailAddress || body.email,
      password: body.password
    };
    if (!loginDto.username || !loginDto.password) {
      return res.status(400).json(responseUtil.getErrorServiceResponse(
        'Send JSON body with username (or emailAddress or email) and password. Use Content-Type: application/json.',
        400
      ));
    }
    const jwtResponse = await userService.login(loginDto);
